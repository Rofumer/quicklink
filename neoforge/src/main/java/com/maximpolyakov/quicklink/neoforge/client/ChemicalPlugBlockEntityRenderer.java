...
public class ChemicalPlugBlockEntityRenderer implements BlockEntityRenderer<ChemicalPlugBlockEntity> {
    ...
    private static final int   X_SEGMENTS  = 11;
    private static final float X_GAP_RATIO = 0.45f;

    public ChemicalPlugBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {}

    @Override
    public void render(ChemicalPlugBlockEntity be, float partialTick, PoseStack poseStack,
        ...
        for (Direction face : Direction.values()) {
            ChemicalPlugBlockEntity.SideRole role = be.getRole(face);
            if (role == ChemicalPlugBlockEntity.SideRole.NONE) continue;
    ...
    private static void drawDashedDiagAxisAligned(VertexConsumer vc, PoseStack.Pose pose, Matrix4f mat,
        ...
        for (int i = 0; i < segs; i++) {
            ...
            float uu0 = clamp01(Math.min(su0, su1) - pad);
            float uu1 = clamp01(Math.max(su0, su1) + pad);
            float vv0 = clamp01(Math.min(sv0, sv1) - pad);
            float vv1 = clamp01(Math.max(sv0, sv1) + pad);
    ...
    private static void drawRectOnFace(VertexConsumer vc, PoseStack.Pose pose, Matrix4f mat,
        ...
        float uu0 = toFaceCoord(Math.min(u0, u1));
        float uu1 = toFaceCoord(Math.max(u0, u1));
        float vv0 = toFaceCoord(Math.min(v0, v1));
        float vv1 = toFaceCoord(Math.max(v0, v1));
        ...
        switch (face) {
            case SOUTH -> quad(vc, pose, mat, nx, ny, nz,
                    uu0, vv0, FACE_MAX + eps,  uu1, vv0, FACE_MAX + eps,  uu1, vv1, FACE_MAX + eps,  uu0, vv1, FACE_MAX + eps,
                    U0, V0, U1, V1, r, g, b, a, light, overlay);

            case NORTH -> quad(vc, pose, mat, nx, ny, nz,
                    1f - uu0, vv0, FACE_MIN - eps,  1f - uu1, vv0, FACE_MIN - eps,  1f - uu1, vv1, FACE_MIN - eps,  1f - uu0, vv1, FACE_MIN - eps,
                    U0, V0, U1, V1, r, g, b, a, light, overlay);

            case EAST -> quad(vc, pose, mat, nx, ny, nz,
                    FACE_MAX + eps, vv0, 1f - uu0,  FACE_MAX + eps, vv0, 1f - uu1,  FACE_MAX + eps, vv1, 1f - uu1,  FACE_MAX + eps, vv1, 1f - uu0,
                    U0, V0, U1, V1, r, g, b, a, light, overlay);

            case WEST -> quad(vc, pose, mat, nx, ny, nz,
                    FACE_MIN - eps, vv0, uu0,  FACE_MIN - eps, vv0, uu1,  FACE_MIN - eps, vv1, uu1,  FACE_MIN - eps, vv1, uu0,
                    U0, V0, U1, V1, r, g, b, a, light, overlay);

            case UP -> quad(vc, pose, mat, nx, ny, nz,
                    uu0, FACE_MAX + eps, 1f - vv0,  uu1, FACE_MAX + eps, 1f - vv0,  uu1, FACE_MAX + eps, 1f - vv1,  uu0, FACE_MAX + eps, 1f - vv1,
                    U0, V0, U1, V1, r, g, b, a, light, overlay);

            case DOWN -> quad(vc, pose, mat, nx, ny, nz,
                    uu0, FACE_MIN - eps, vv0,  uu1, FACE_MIN - eps, vv0,  uu1, FACE_MIN - eps, vv1,  uu0, FACE_MIN - eps, vv1,
                    U0, V0, U1, V1, r, g, b, a, light, overlay);
        }
    }
    ...
    private static int boostLight(int packedLight, int addBlock, int addSky) {
        ...
        int block = Math.min(15, LightTexture.block(packedLight) + Math.max(0, addBlock));
        int sky = Math.min(15, LightTexture.sky(packedLight) + Math.max(0, addSky));
