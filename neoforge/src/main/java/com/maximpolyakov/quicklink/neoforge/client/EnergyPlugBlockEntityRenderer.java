...
public class EnergyPlugBlockEntityRenderer implements BlockEntityRenderer<EnergyPlugBlockEntity> {
    ...
    @Override
    public void render(EnergyPlugBlockEntity be, float partialTick, PoseStack poseStack,
        ...
        for (Direction face : Direction.values()) {
            ...
            if (role == EnergyPlugBlockEntity.SideRole.NONE) continue;

            boolean on = be.isSideEnabled(face);

            if (on) {
                if (role == EnergyPlugBlockEntity.SideRole.PLUG) {
