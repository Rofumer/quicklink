...
public class FluidPlugBlockEntityRenderer implements BlockEntityRenderer<FluidPlugBlockEntity> {
    ...
    @Override
    public void render(FluidPlugBlockEntity be, float pt, PoseStack ps, MultiBufferSource buf, int pl, int po) {
        ...
        for (Direction face : Direction.values()) {
            FluidPlugBlockEntity.SideRole role = be.getRole(face); if (role == FluidPlugBlockEntity.SideRole.NONE) continue;
    ...
    private static void draw4(VertexConsumer vc, Matrix4f mat, Matrix3f norm, Direction face, float eps,
                              float U0, float U1, float V0, float V1, byte[] c, int light, int ov) {
        float I = QUAD_INSET;
    ...
    private static void drawQ(VertexConsumer vc, Matrix4f mat, Matrix3f norm, Direction face,
                              float u0, float u1, float v0, float v1, float eps,
                              float U0, float U1, float V0, float V1, byte dyeId, int light, int ov) {
    ...
    private static void drawLeg(VertexConsumer vc, Matrix4f mat, Matrix3f norm, Direction face, boolean outward,
                                float r, float g, float b, float a, float uOff,
                                float U0, float U1, float V0, float V1, int light, int ov) {
    ...
    private static void drawX(VertexConsumer vc, Matrix4f mat, Matrix3f norm, Direction face, float eps,
                              float U0, float U1, float V0, float V1,
                              float r, float g, float b, float a, float thick, int light, int ov) {
        drawXCell(vc,mat,norm,face,eps,U0,U1,V0,V1,NORM_A0,NORM_A1,NORM_A0,NORM_A1,r,g,b,a,thick,light,ov);
        drawXCell(vc,mat,norm,face,eps,U0,U1,V0,V1,NORM_B0,NORM_B1,NORM_A0,NORM_A1,r,g,b,a,thick,light,ov);
        drawXCell(vc,mat,norm,face,eps,U0,U1,V0,V1,NORM_A0,NORM_A1,NORM_B0,NORM_B1,r,g,b,a,thick,light,ov);
        drawXCell(vc,mat,norm,face,eps,U0,U1,V0,V1,NORM_B0,NORM_B1,NORM_B0,NORM_B1,r,g,b,a,thick,light,ov);
    }
    private static void drawXCell(VertexConsumer vc, Matrix4f mat, Matrix3f norm, Direction face, float eps,
                                  float U0,float U1,float V0,float V1,float cu0,float cu1,float cv0,float cv1,
                                  float r,float g,float b,float a,float thick,int light,int ov){
    ...
    private static void diag(VertexConsumer vc,Matrix4f mat,Matrix3f norm,Direction face,float eps,
                              float U0,float U1,float V0,float V1,float r,float g,float b,float a,float thick,int light,int ov,
                              float uA,float vA,float uB,float vB){
        float gap=clamp01(X_GAP); float pad=thick*0.5f;
        for(int i=0;i<X_SEGS;i++){
            float t0=(float)i/X_SEGS,t1=(float)(i+1)/X_SEGS,cut=(t1-t0)*gap*0.5f;
            t0+=cut;t1-=cut;if(t1<=t0)continue;
            float su0=lerp(uA,uB,t0),sv0=lerp(vA,vB,t0),su1=lerp(uA,uB,t1),sv1=lerp(vA,vB,t1);
            rect(vc,mat,norm,face,eps,clamp01(Math.min(su0,su1)-pad),clamp01(Math.max(su0,su1)+pad),
                    clamp01(Math.min(sv0,sv1)-pad),clamp01(Math.max(sv0,sv1)+pad),U0,U1,V0,V1,r,g,b,a,light,ov);
        }
    }

    private static void rect(VertexConsumer vc, Matrix4f mat, Matrix3f norm, Direction face, float eps,
                             float u0, float u1, float v0, float v1,
                             float U0, float U1, float V0, float V1,
                             float r, float g, float b, float a, int light, int ov) {
    ...
    private static void q4(VertexConsumer vc,Matrix4f mat,Matrix3f norm,float nx,float ny,float nz,
                           float[]v1,float[]v2,float[]v3,float[]v4,float U0,float V0,float U1,float V1,
                           float r,float g,float b,float a,int light,int ov){
        q4f(vc,mat,norm,nx,ny,nz,v1[0],v1[1],v1[2],v2[0],v2[1],v2[2],v3[0],v3[1],v3[2],v4[0],v4[1],v4[2],U0,V0,U1,V1,r,g,b,a,light,ov);
    }

    private static void q4f(VertexConsumer vc, Matrix4f mat, Matrix3f norm,
    ...
    private static void v(VertexConsumer vc, Matrix4f mat, Matrix3f norm,
                          ...
                          float nx, float ny, float nz, int light, int ov) {
