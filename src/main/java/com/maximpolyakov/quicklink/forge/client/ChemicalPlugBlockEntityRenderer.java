...
public class ChemicalPlugBlockEntityRenderer implements BlockEntityRenderer<ChemicalPlugBlockEntity> {
    ...
    @Override
    public void render(ChemicalPlugBlockEntity be, float pt, PoseStack ps, MultiBufferSource buf, int pl, int po) {
        ...
        for (Direction face : Direction.values()) {
            ChemicalPlugBlockEntity.SideRole role = be.getRole(face); if(role==ChemicalPlugBlockEntity.SideRole.NONE) continue;
    ...
    private static void drawXCell(VertexConsumer vc,Matrix4f mat,Matrix3f norm,Direction face,float eps,float U0,float U1,float V0,float V1,float cu0,float cu1,float cv0,float cv1,float r,float g,float b,float a,float thick,int light,int ov){
